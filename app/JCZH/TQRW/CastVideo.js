define(function(require, exports, module) {
    ...
    function(Device, $scope, $routeParams, specialDuty, Buffer, Rest) {
        ...
        socket.emit('subscribe',{
            clientType:'video',
            id: $routeParams.id + session + 'signalVideosCast'
        });

        socket.on('videoDispatch:' + $routeParams.id + session+'signalVideosCast', function(data) {
            // console.log('CastVideo-videoDispatch-on:', data);
            if(!(data.videos && data.videos.config && data.videos.config.videos)) {
                data.videos.config.videos = [];
            }
        ...
        // socket.on('tq:' + $routeParams.id + ':signalVideosCast:' + session, function(data) {
        //     $scope.selected = data.videos;
        //     $scope.total = $scope.selected.config.videos.length;
        //     $scope.$apply();
        //     console.log($scope.total, $scope.selected);
